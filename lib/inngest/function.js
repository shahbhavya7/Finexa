  ...
  async ({ step }) => {
    ...
    for (const user of users) { // Iterate through each user to generate reports
      await step.run(`generate-report-${user.id}`, async () => { // Run a step for each user to generate their report
        ...
        const insights = await generateFinancialInsights(stats, monthName); // Generate insights using AI based on the stats

        await sendEmail({ // Send the monthly report email to the user
          to: user.email,
    ...
    return { processed: users.length }; // Return the number of users processed
  }
);
